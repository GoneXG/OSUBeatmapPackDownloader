...
- 下载期间显示一个原地刷新的进度块：**第一行**是整体进度（百分比、完成数/总数、总字节、总速度与整体剩余时间），**下面每个正在下载的曲包各占一行**，可以看到该曲包自身的百分比、已下载/总字节、速度与剩余时间：
  ...
  ```text
  [######----------------]  30%  6/20  DL:3  60.4MiB/201.0MiB  6.7MiB/s  ETA 00:21
    45% [###-----] SM379 - osu!mania Beatmap Pack #379.zip  45.8MiB/101.0MiB  3.1MiB/s  ETA 00:18
    16% [#-------] SM378 - osu!mania Beatmap Pack #378.zip  14.6MiB/91.0MiB  3.6MiB/s  ETA 00:21
  ```

  （数字为示意；实际行数等于当前**已经建立连接、正在传输**的曲包数量，aria2 最多 8 个并发；还排在队列里、或服务器还没响应的曲包不会占一行，下载完成的曲包其行也会自动消失。）输出被重定向到文件或管道时，会按周期输出同样的整块文本、每行一个曲包，不写回车等控制字符；双击运行在很旧的控制台上如果无法原地刷新多行，会自动退化为单行整体进度。
...
- **部分曲包直链下载失败（例如老曲包 404）**：程序会带上 Cookie 并发访问这些曲包的 `?format=raw` 详情页（并发数见 `-lookup-concurrency`，默认 4），解析页面中的真实下载地址后重新下载，并**一轮轮继续**：每轮只处理当前仍失败的曲包，本轮没查到地址的曲包会在下一轮重新查询，已经下载成功的曲包不再重复下载。只有某一轮没有任何曲包被重新下载成功（无进展）时才停止；Cookie 连续 3 次仍提示需要登录、用户直接回车跳过、或出现网络层错误时也会停止。每轮开始与结束都会打印一行进度，例如 `第 2 轮：剩余 3 个失败曲包，开始重查官方存储地址（并发 4）...`，因此可以清楚看到程序仍在继续拉取剩余曲包。
...
A：程序会多轮重查重下，直到某一轮毫无进展才停下，所以「只恢复了一部分就退出」通常说明剩余曲包要么官网确实不提供地址（极少数很老的曲包已从官方存档下架，404 属于正常现象），要么当轮下载没有取得任何进展。查看 `URL/failed.txt` 里的记录确认剩余项，再次运行程序选择同样的分类即可继续尝试这些曲包（已下载完成的 zip 不会重下）。
